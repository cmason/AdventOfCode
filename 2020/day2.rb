...
def valid_password2?(min:, max:, char:, password:)
  [password[min.to_i - 1 ], password[max.to_i - 1]].one?(char)
end

def parse_string(string)
  string.match(PASSWORD_REGEX).named_captures.transform_keys(&:to_sym)
end

puts DATA.readlines.count { |line|
  valid_password2? parse_string(line)
}
__END__
2-6 w: wkwwwfwwpvw
