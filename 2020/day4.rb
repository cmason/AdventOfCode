...
def valid_hgt?(hgt)
  height = hgt.to_i
  unit = hgt[-2..]
  return true if unit == 'cm' && height.between?(150, 193)
  return true if unit == 'in' && height.between?(59, 76)
  false
end
...
def valid_ecl?(ecl)
  %w[amb blu brn gry grn hzl oth].any?(ecl)
end
...
def valid?(passport)
  VALID_FIELDS.all? { |fld| send("valid_#{fld}?", passport.fetch(fld, '')) }
end

p DATA.read.split("\n\n").map { |str| str.split(' ').map { |kv| kv.split(':')}.to_h}.count { |p| valid? p }
__END__
